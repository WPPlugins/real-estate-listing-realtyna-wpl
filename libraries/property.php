...
class wpl_property
{
	...
	public static function generate_location_text($property_data, $property_id = 0, $glue = ',', $force = false, $only_return = false)
	{
        ...
        // Get the pattern
        $default_pattern = '[street_no] [street] [street_suffix][glue] [location4_name][glue] [location2_name] [zip_name]';
    ...
    public static function location_visibility($property_id, $kind = NULL, $membership_id = NULL)
    {
        ...
        if(!$show_address) return __($dbst['accesses_message'], 'wpl');
     ...
     * @param $property_id
     * @param string $stats_type | $item_type
     *
     * @return void
     */
    ...
    {
        /** first validation **/
        if(!trim($property_id) or empty($stats_type)) return;
        ...
        if($current_user_id != $property_user)
        {
            $exists = wpl_items::get_items($property_id, $stats_type);

            if(!empty($exists)) wpl_db::q("UPDATE `#__wpl_items` SET `item_name`=(`item_name`+1) WHERE `parent_id`='{$property_id}' AND `item_type`='{$stats_type}'");
            else
            {
                $item_array = array(
                    'parent_id'=>$property_id,
                    'item_type'=>$stats_type,
                    'item_cat'=>'stats',
                    'item_name'=>1
                );
                wpl_items::insert($item_array);
            }
        }
    }
     ...
     * @return integer
     */
    public static function get_all_visits($blog_id = 0)
    {
        $result = 0;
        $where = '';
        $current_user_id = wpl_users::get_cur_user_id();
        ...
        {
            $where .= "AND `user_id` = '{$current_user_id}' ";
        }
        ...
        if(wpl_global::check_addon('franchise'))
        {
            ...
            $query = "SELECT SUM(`visits`) FROM `#__wpl_property_visits` WHERE 1 {$where}";
            $result = wpl_db::select($query, 'loadResult');
        }
        else
        {
            $query = "SELECT SUM(`visit_time`) FROM `#__wpl_properties` WHERE 1 {$where}";
            $result = wpl_db::select($query, 'loadResult');
        }
