...
if($type == 'seo_patterns' and !$done_this)
{
    ...
    $patterns = array(
        'property_alias_pattern'=>__('Listing Link Pattern', 'wpl'),
        'property_title_pattern'=>__('Listing Title', 'wpl'),
        'property_page_title_pattern'=>__('Listing Page Title', 'wpl'),
        'meta_description_pattern'=>__('Meta Description', 'wpl'),
        'meta_keywords_pattern'=>__('Meta Keywords', 'wpl')
    );
?>
                                    ...
                                    <?php foreach($property_types as $property_type): ?>
                                        <div class="wpl-gen-accordion">
                                            <h4 class="wpl-gen-accordion-title" data-for="wpl_seo_patterns_ptype<?php echo $kind['id']; ?>_<?php echo $property_type['id']; ?>">
                                                <span><?php echo __($property_type['name'], 'wpl'); ?></span>
                                            </h4>
...
function wpl_seo_patterns_save()
{
	...
	wplj.ajax(
	{
		...
		{
            wpl_show_messages('<?php echo __('Error Occured.', 'wpl'); ?>', '#wpl_seo_patterns_show_message', 'wpl_red_msg');
			wplj(ajax_loader_element).html('');
		}
	});
}
