...
class wpl_events_notifications
{
    ...
    public static function contact_agent($params)
    {
        ...
        if(wpl_global::check_addon('crm'))
        {
            ...
            $contact = new wpl_addon_crm_contact();
            if(isset($replacements['user_id']))
            {
                $data = ['first_name' => $replacements['fullname'], 'email' => $replacements['email'], 'tel' => $replacements['phone'],'owner'=>$replacements['user_id']];
                $cid = $contact->save($data);
            }
        }
        return true;
    }
