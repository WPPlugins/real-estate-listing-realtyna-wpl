...
<div class="fanc-body">
	...
	<div class="col-wp">
		...
		<div class="col-fanc-right" id="wpl_flex_specific_options">
			...
			<div class="fanc-row">
				<label for="<?php echo $__prefix; ?>opt_parent_kind"><?php echo __('Parent Kind', 'wpl'); ?></label>
                <select name="<?php echo $__prefix; ?>opt_parent_kind" id="<?php echo $__prefix; ?>opt_parent_kind">
                    <?php foreach($kinds as $kind): ?>
                    <option <?php echo (isset($options['parent_kind']) and $options['parent_kind'] == $kind['id']) ? 'selected="selected"' : ''; ?> value="<?php echo $kind['id']; ?>"><?php echo __($kind['name'], 'wpl'); ?></option>
                    <?php endforeach; ?>
                </select>
			</div>
            <div class="fanc-row">
				<label for="<?php echo $__prefix; ?>opt_replace"><?php echo __('Replace Parent Data', 'wpl'); ?></label>
                <select name="<?php echo $__prefix; ?>opt_replace" id="<?php echo $__prefix; ?>opt_replace">
                    <option <?php echo (isset($options['replace']) and $options['replace'] == 1) ? 'selected="selected"' : ''; ?> value="1"><?php echo __('Yes', 'wpl'); ?></option>
                    <option <?php echo (isset($options['replace']) and $options['replace'] == 0) ? 'selected="selected"' : ''; ?> value="0"><?php echo __('No', 'wpl'); ?></option>
                </select>
			</div>
		</div>
	</div>
