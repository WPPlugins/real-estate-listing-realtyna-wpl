...
class wpl_flex
{
    ...
    public static function get_field_by_column($column, $kind)
	{
		...
		$query = "SELECT * FROM `#__wpl_dbst` WHERE `kind`='$kind'";
		$results = wpl_db::select($query, 'loadObjectList');

        $fields = array();
        foreach($results as $result)
        {
            if(!isset($fields[$kind])) $fields[$kind] = array();
            if(!trim($result->table_column)) continue;
            
            $fields[$kind][$result->table_column] = $result;
        }

        /** add to cache **/
		self::$fields_by_column = array_merge_recursive(self::$fields_by_column, $fields);
