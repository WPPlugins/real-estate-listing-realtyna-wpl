	...
	foreach($sort_options as $sort_option)
	{
        if(in_array($sort_option['field_name'], array('ptype_adv', 'ltype_adv')))
        {
            ...
            foreach($types as $type)
            {
                ...
                if(!isset($sort_option['asc_enabled']) or (isset($sort_option['asc_enabled']) and $sort_option['asc_enabled'])) $html .= '<option value="wplorderby='.urlencode($sort_option['field_name'].':'.$type['id']).'&amp;wplorder=ASC" '.(($this->orderby == "(p.`".$column."` != '".$type['id']."'), p.`".$column."`") ? 'selected="selected"' : '').'>'.sprintf(__('%s first', 'wpl'), __(wpl_global::pluralize(2, $type['name']), 'wpl')).'</option>';
                if(!isset($sort_option['desc_enabled']) or (isset($sort_option['desc_enabled']) and $sort_option['desc_enabled'])) $html .= '<option value="wplorderby='.urlencode($sort_option['field_name'].':'.$type['id']).'&amp;wplorder=DESC" '.(($this->orderby == "(p.`".$column."` = '".$type['id']."'), p.`".$column."`") ? 'selected="selected"' : '').'>'.sprintf(__('%s last', 'wpl'), __(wpl_global::pluralize(2, $type['name']), 'wpl')).'</option>';
            }
        }
        else
        {
            $asc_label = sprintf(__('%s ascending', 'wpl'), __($sort_option['name'], 'wpl'));
            $desc_label = sprintf(__('%s descending', 'wpl'), __($sort_option['name'], 'wpl'));
            
            if(isset($sort_option['asc_label']) and trim($sort_option['asc_label'])) $asc_label = __($sort_option['asc_label'], 'wpl');
            if(isset($sort_option['desc_label']) and trim($sort_option['desc_label'])) $desc_label = __($sort_option['desc_label'], 'wpl');
...
elseif($type == 1)
{
	...
	foreach($sort_options as $sort_option)
	{
        ...
        else
        {
            ...
            $label = __($sort_option['name'], 'wpl');
            
            if($current_order == 'ASC' and isset($sort_option['asc_label']) and trim($sort_option['asc_label'])) $label = __($sort_option['asc_label'], 'wpl');
            if($current_order == 'DESC' and isset($sort_option['desc_label']) and trim($sort_option['desc_label'])) $label = __($sort_option['desc_label'], 'wpl');
