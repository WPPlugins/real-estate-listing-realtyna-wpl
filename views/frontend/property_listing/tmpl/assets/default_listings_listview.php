...
<div class="wpl_sort_options_container">
    ...
    <span class="wpl-sort-options-list"><?php echo $this->model->generate_sorts(array('type'=>1, 'kind'=>$this->kind, 'sort_options'=>$sort_options)); ?></span>
    <span class="wpl-sort-options-selectbox"><?php echo $this->model->generate_sorts(array('type'=>0, 'kind'=>$this->kind, 'sort_options'=>$sort_options)); ?></span>
    <?php endif; ?>
    ...
    <div class="wpl_list_grid_switcher <?php if($this->switcher_type == "icon+text") echo 'wpl-list-grid-switcher-icon-text'; ?>">
        ...
        <?php if(wpl_global::check_addon('aps') and (!isset($this->settings['googlemap_display_status']) or (isset($this->settings['googlemap_display_status']) and $this->settings['googlemap_display_status']) or (isset($this->settings['googlemap_display_status']) and !$this->settings['googlemap_display_status'] and wpl_request::getVar('wplfmap', 0)))): ?>
            ...
            <?php if ($this->switcher_type == "icon"): ?>
                <div class="wpl-util-hidden"><?php _e('Map', 'wpl') ?></div>
            <?php endif; ?>

        <?php elseif(wpl_global::check_addon('aps')): ?>
			<a class="<?php echo ($this->switcher_type == "icon") ? 'wpl-tooltip-top ' : ''; ?>map_view" href="<?php echo wpl_global::add_qs_var('wplpcc', 'map_box', wpl_global::add_qs_var('wplfmap', 1,$listings_page)); ?>">
				<?php if($this->switcher_type == "icon+text") echo '<span>'.__('Map', 'wpl').'</span>'; ?>
			</a>
            <?php if ($this->switcher_type == "icon"): ?>
                <div class="wpl-util-hidden"><?php _e('Map', 'wpl') ?></div>
