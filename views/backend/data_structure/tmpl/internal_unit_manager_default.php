...
<div class="unit-manager-wp">
    <div class="panel-wp">
        <div class="panel-body">
            <span><?php echo __('Unit type', 'wpl'); ?> : </span>
            <select class="selectbox" onchange="load_new_unit_category(this.value);">
                <?php foreach ($this->unit_types as $id => $wp_unite_type): ?>
                <option value="<?php echo $wp_unite_type['id']; ?>" <?php if ($wp_unite_type['id'] == 4) echo 'selected="selected"'; ?>><?php echo __($wp_unite_type['name'], 'wpl'); ?></option>
                <?php endforeach; ?>
            </select>
