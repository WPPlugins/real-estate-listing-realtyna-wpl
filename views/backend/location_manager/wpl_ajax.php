...
class wpl_location_manager_controller extends wpl_controller
{
	...
	public function display()
	{
        ...
        // Check Nonce
        if(!wpl_security::verify_nonce(wpl_request::getVar('_wpnonce', ''), 'wpl_location_manager')) $this->response(array('success'=>0, 'message'=>__('The security nonce is not valid!', 'wpl')));
	...
	private function save_location($name, $abbr, $level, $parent, $location_id = '')
	{
		...
		$res = 1;
		$message = $res ? __('Location saved.', 'wpl') : __('Error Occured.', 'wpl');
		$data = NULL;
	...
	private function delete_location($level, $location_id)
	{
		...
		$res = $res ? 1 : 0;
		$message = $res ? __('Location(s) Deleted.', 'wpl') : __('Error Occured.', 'wpl');
		$data = NULL;
