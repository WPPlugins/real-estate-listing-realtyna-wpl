...
<div class="wrap wpl-wp location-wp">
    <header>
        ...
        <h2><?php echo __('Locations', 'wpl'); ?></h2>
        <a data-realtyna-lightbox data-realtyna-lightbox-opts="clearContent:false" href="#wpl_location_settings_lightbox" class="action-btn icon-gear"><?php echo __('Settings', 'wpl'); ?></a>
    </header>
    <div class="sidebar-wp">
        <div class="side-15">
            <div class="panel-wp">
                <h3>
                    <?php echo __('Location Tools', 'wpl'); ?>
                </h3>
                <div class="panel-body">
                    ...
                    <div class="location_tools">
                        ...
                        <input class="button" type="submit" value="<?php echo __('Show all countries', 'wpl'); ?>" onclick="wpl_show_countries(-1);" />
                        <?php elseif ($this->level == 1): ?>
                        <input class="button" type="submit" value="<?php echo __('Show enabled countries', 'wpl'); ?>" onclick="wpl_show_countries(1);" />
                        <?php endif; ?>
                        <input class="text_box" type="text" id="wpl_search_location" value="<?php echo $this->text_search; ?>" autocomplete="on" />
                        <input class="button" type="submit" value="<?php echo __('Search', 'wpl'); ?>" onclick="wpl_search_location('wpl_search_location', '<?php echo $this->level; ?>');" />
                        <input class="button" type="submit" value="<?php echo __('Reset', 'wpl'); ?>" onclick="wpl_reset_search_location();" />
                    </div> 
                </div>
            </div>
            ...
            <table class="widefat page fixed">
                <thead>
                    <tr>
                        <th scope="col" class="manage-column" class="size-1"><?php echo wpl_global::order_table(__('ID', 'wpl'), 'id'); ?></th>
                        <th scope="col" class="manage-column" class="size-4"><?php echo wpl_global::order_table(__('Name', 'wpl'), 'name'); ?></th>
                ...
                <tbody>
                    ...
                    <tr>
                    	<td onclick="wplj('#wpl_add_location_item').trigger('click');" class="wpl_no_item" colspan="5"><?php echo __('No location! Add a new location', 'wpl'); ?></td>
                    </tr>
                    <?php endif; ?>
                    <?php
                    ...
                    <tr id="item_row<?php echo $wp_location->id; ?>">
                        ...
                        <td>
                            ...
                            if ($this->level < 7 and !$this->load_zipcodes)
                                echo '<a href="' . $link . '">' . __('Load Next Level', 'wpl') . '</a>';
                            if ($this->level == $this->zipcode_parent_level)
                                echo '<br /><a href="' . $link . '&load_zipcodes=1">' . __('Load Zipcodes', 'wpl') . '</a>';
                            ?>
                        </td>
            ...
            <div id="wpl_location_settings_lightbox" class="wpl_hidden_element">
                <div class="fanc-content size-width-1 fanc-settings">
                    <h2><?php echo __('Location Settings', 'wpl'); ?></h2>
